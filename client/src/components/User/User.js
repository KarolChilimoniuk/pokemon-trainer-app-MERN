...
const User = () => {
  ...
  return (
    <section className={styles.user__container}>
      {loggedUser ? (
        <h3 className={styles.header}>User panel</h3>
      ) : (
        <h3>Something wrong :/</h3>
      )}
      <p>
        <span className={styles.span}>user-name:</span> {userData.userName}
      </p>
      <p>
        <span className={styles.span}>E-mail:</span> {userData.email}
      </p>
      <p>
        <span className={styles.span}>Added trainers:</span>{" "}
        {userData.trainers.length === 0 ? "None" : userTrainers}
      </p>
      <button
