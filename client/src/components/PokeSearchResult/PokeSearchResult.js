...
const PokeSearchResult = ({textToSearch}) => {
    ...
    useEffect(() => {
      const monster = pokemons.filter(el => el.name.toLowerCase() === textToSearch.toLowerCase());
      monster.length > 0 ? newMonsterToDisplay(monster[0]) : newMonsterToDisplay('');
    }, [textToSearch]);

   return (
    <section className={styles.result__container}>
      {monsterToDisplay !== '' ? <PokeListElement monster={monsterToDisplay} inAddPokeComponent={true} inTrainerComponent={false}/> : null}
    </section>
   )
}
