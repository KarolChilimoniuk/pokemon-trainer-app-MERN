...
//    const [modalStyles, newModalStyle] = useState(show ? styles.addPokemonModal__visible : styles.addPokemonModal__hidden);
   ...
   const modalStyles = showModal ? styles.addPokemonModal__visible : styles.addPokemonModal__hidden;
   ...
   const submitHandler = (e) => {
     e.preventDefault();
     newPokemonName(inputText);
   }
   
   return (
    <section className={modalStyles}>
        <form className={styles.addPokemonModal__form} onSubmit={submitHandler}>
            <img className={styles.addPokemonModal__img} src={CrossImage} onClick={() => showModalHandler(!showModal)}/>
            <div className={styles.addPokemonModal__inputContainer}>
                <label>Search Pokemon: </label>
                <input className={styles.addPokemonModal__textInput} type="text" placeholder="Write Pokemon name" onChange={textHandler}/>
            </div>
            <input className={styles.addPokemonModal__submitInput} type="submit" value="Search Pokemon!"/>
